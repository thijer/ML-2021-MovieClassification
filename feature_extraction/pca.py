...
def load_img(id):
    return imread(path_data + "normal/{}.jpg".format(id[0])).astype(np.int16).flatten()

def main(pca_path = None):
    pca = IncrementalPCA(components, batch_size = batch_size)
       
    data = pd.read_csv(path_data + "41K_processed_v3.csv", index_col='id')
...
def fit(pca: IncrementalPCA, ids):
    ...
    for i in range(0, (batches - 1) * batch_size, batch_size):
        ...
        # imgs[i] = imread(path_data + "normal\\{}.jpg".format()).flatten()
    ...
    if(components != None): plt.savefig(path_figures + "Scree_{}_components.png".format(components))
    else: plt.savefig(path_figures + "Scree_all_components.png")
...
if __name__ == '__main__':
    ...
    # components = 1024
    # path_data = os.getcwd() + "/data/duplicate_free_41K.csv"
    main()